...
export const RotorSelectModal: FunctionComponent<RotorSelectModalProps> = ({ modalVisible, setModalVisible, setRotor, currentRotor }) => {
    ...
    const chooseRotor = (rotor: RotorState) => {
        ...
        if (currentRotor) dispatch(updateRotor({id: currentRotor.id, isAvailable: true}));
        setRotor(rotor);
        closeModal();
    }
    return (
        ...
        >
            <View>
                ...
                {rotors.filter((rotor) => rotor.isAvailable).map((rotor) => {
                    return (
                        <Button key={rotor.id} onPress={() => chooseRotor(rotor)}>{rotor.id}</Button>
                    )})
                }
            </View>
        </Modal>
