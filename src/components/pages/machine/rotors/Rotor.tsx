...
export const Rotor: FunctionComponent = () => {
    ...
    const removeRotor = () => {
        if (selectedRotor) dispatch(updateRotor({id: selectedRotor.id, isAvailable: true}));
        setSelectedRotor(null);
    }
    useEffect(() => {
        if (selectedRotor) {
            setSelectedRotor(rotors.filter((rotor) => rotor.id === selectedRotor.id)[0]);
        }
    }, [rotors])
    return (
        <View>
            <Portal>
                <RotorSelectModal modalVisible={isSelectModalOpen} setModalVisible={setIsSelectModalOpen} setRotor={setSelectedRotor} currentRotor={selectedRotor} />
                {selectedRotor && <ChangeIndexModal modalVisible={isChangeIndexModalOpen} setModalVisible={setIsChangeIndexModalOpen} setRotor={setSelectedRotor} currentRotor={selectedRotor} />}
            </Portal>
            <Card style={rotorStyles.rotor} contentStyle={{paddingHorizontal: 0}} >
                {selectedRotor && (
                    <>
                        <Card.Title
                            title={`Current letter: ${selectedRotor.config.displayedLetters[selectedRotor.config.currentIndex]}`}
                ...
                {selectedRotor === null && (
                    <>
                        <Card.Title
                            ...
                            right={() => <Button mode='contained' onPress={setRotor}>Select rotor</Button>}
                        />
                    </>
                )}
            </Card >
